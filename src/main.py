    ...
    def command(node_id):
        try:
            if not if_exist(node_id):
                error_print(name, f"Node with ID {node_id} does not exist")

            time_start = time.time()
            results = dgraph_read(query, variables={"$id": node_id})
            time_end = time.time()

            length_print(name, results)
            json_print(results)
...
def find_shortest_path(node_id1, node_id2):
    ...
    try:
        __myname__ = "find_shortest_path"
        if not if_exist(node_id1):
            error_print(__myname__, f"Node {node_id1} does not exist")

        if not if_exist(node_id2):
            error_print(__myname__, f"Node {node_id2} does not exist")

        start_time = time.time()
        results = shortest_path(node_id1, node_id2)
        end_time = time.time()

        length_print(__myname__, results)
        json_print(results)
        verbose_print(f"Query executed in {end_time - start_time:.2f} seconds")
    except Exception as error:
